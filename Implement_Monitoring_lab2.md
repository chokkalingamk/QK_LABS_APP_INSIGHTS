---
lab:
    title: '2 - Implement Monitoring'
    module: 'Administer Monitoring'
---

# Lab 2 - Implement Monitoring
# Student lab manual
...
1. In the toolbar of the Cloud Shell pane, click the **Upload/Download files** icon, in the drop-down menu, click **Upload** and upload the files **\\Labs\\2\\az104-2-vm-template.json** and **\\Labs\\2\\az104-2-vm-parameters.json** into the Cloud Shell home directory.
...
1. From the Cloud Shell pane, run the following to create the first virtual network and deploy a virtual machine into it by using the template and parameter files you uploaded:
   ...
   New-AzResourceGroupDeployment `
      ...
      -TemplateFile $HOME/az104-2-vm-template.json `
      -TemplateParameterFile $HOME/az104-2-vm-parameters.json `
      -AsJob
   ```
...
1. On the **Basics** tab of the **Create Log Analytics workspace** blade, enter the following settings, click **Review + Create** and then click **Create**:
    ...
    | Resource group | the name of a new resource group **az104-2-rg1** |
...
1. In the Azure portal, search for and select **Virtual machines**, and on the **Virtual machines** blade, click **az104-2-vm0**.

1. On the **az104-2-vm0** blade, in the **Monitoring** section, click **Metrics**.

1. On the **az104-2-vm0 \| Metrics** blade, on the default chart, note that the only available **Metrics Namespace** is **Virtual Machine Host**.
...
1. On the **az104-2-vm0** blade, in the **Monitoring** section, click **Diagnostic settings**.

1. On the **Overview** tab of the **az104-2-vm0 \| Diagnostic settings** blade, click **Enable guest-level monitoring**.

    >**Note**: Wait for the operation to take effect. This might take about 3 minutes.

1. Switch to the **Performance counters** tab of the **az104-2-vm0 \| Diagnostic settings** blade and review the available counters.
...
1. Switch to the **Logs** tab of the **az104-2-vm0 \| Diagnostic settings** blade and review the available event log collection options.
...
1. On the **az104-2-vm0** blade, in the **Monitoring** section, click **Log Analytics Agent** and then click **Enable**.

1. On the **az104-2-vm0 - Logs** blade, ensure that the Log Analytics workspace you created earlier in this lab is selected in the **Choose a Log Analytics Workspace** drop-down list and click **Enable**.
...
1. On the **az104-2-vm0 \| Logs** blade, in the **Monitoring** section, click **Metrics**.

1. On the **az104-2-vm0 \| Metrics** blade, on the default chart, note that at this point, the **Metrics Namespace** drop-down list, in addition to the **Virtual Machine Host** entry includes also the **Guest (classic)** entry.
...
1. On the **Select a scope** blade, on the **Browse** tab, navigate to the **az104-2-rg0** resource group, expand it, select the checkbox next to the **az104-2-vm0** virtual machine entry within that resource group, and click **Apply**.

    >**Note**: This gives you the same view and options as those available from the **az104-2-vm0 - Metrics** blade.

1. In the **Metric** drop-down list, select **Percentage CPU**, in the **Aggregation** drop-down list, select **Avg**, and review the resulting chart.

1. On the **Monitor \| Metrics** blade, on the **Avg Percentage CPU for az104-2-vm0** pane, click **New alert rule**.
...
1. On the **Basics** tab of the **Create action group** blade, specify the following settings (leave others with their default values) and select **Next: Notifications >**:
    ...
    | Subscription | the name of the Azure subscription you are using in this lab |
    | Resource group | **az104-2-rg1** |
    | Action group name | **az104-2-ag1** |
    | Display name | **az104-2-ag1** |
...
1. On the **az104-2-vm0** blade, click **Connect**, in the drop-down menu, click **RDP**, on the **Connect with RDP** blade, click **Download RDP File** and follow the prompts to start the Remote Desktop session.
...
1. From the Command Prompt, run the following to trigger increased CPU utilization on the **az104-2-vm0** Azure VM:
...
1. If necessary, click **Select scope**, on the **Select a scope** blade, select the **Recent** tab, select **az104-2-vm0**, and click **Apply**.
...
1. List all resource groups created throughout the labs of this module by running the following command:
   ...
   ```powershell
   Get-AzResourceGroup -Name 'az104-2*'
   ```

1. Delete all resource groups you created throughout the labs of this module by running the following command:

   ```powershell
   Get-AzResourceGroup -Name 'az104-2*' | Remove-AzResourceGroup -Force -AsJob
   ```
